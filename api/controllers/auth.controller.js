...
export const signup = async (req, res, next) => {
  ...
  const stockImgURL = "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png";
  ...
  const sql = "INSERT INTO user_tbl (username, email, password, role, img_url, createdon, updatedon) VALUES (?, ?, ?, ?, ?, ?, ?)";
  const values = [username, email, hashedPassword, 0, stockImgURL, dateNow, dateNow];
...
export const google = async (req, res) => {
  ...
  if(checkUser === undefined){
    ...
    const dateNow = new Date();
    const sql = "INSERT INTO user_tbl (username, email, password, role, img_url, createdon, updatedon) VALUES (?, ?, ?, ?, ?, ?, ?)";
    const newName = name.split(" ").join("").toLowerCase() + Math.floor(Math.random() * 10000).toString();
