...
export const signup = async (req, res, next) => {
    ...
    const sql = "INSERT INTO user_tbl (username, email, password, role, createdon, updatedon) VALUES (?, ?, ?, ?, ?, ?)";
...
export const signin = async (req, res, next) => {
    ...
    db.query(sql, value, (err, result) => {
        if(err) {
            ...
            res.cookie('access_token', token, { httpOnly: true, expire: exp }).status(200).json(validUser);
        }
    });
}